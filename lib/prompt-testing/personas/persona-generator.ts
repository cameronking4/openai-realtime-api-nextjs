...
import { anthropicClient } from '../../anthropic-client'; // Assuming you have an Anthropic client in your main app
...
export class PersonaGenerator {
  ...
  async generatePersona(options: PersonaGenerationOptions = {}): Promise<PatientPersona> {
    let personaData: Omit<PatientPersona, 'id'>;
    
    if (this.useMockResponses) {
      // Use mock data for testing
      const simulatedLLMResponse = this.simulateClaudeResponse(options);
      personaData = this.parsePersonaFromText(simulatedLLMResponse);
    } else {
      // Use actual Claude API
      const prompt = this.buildPrompt(options);
      const response = await this.callClaudeAPI(prompt);
      personaData = this.parsePersonaFromText(response);
    }
  ...
  /**
   * Call the Claude API to generate a persona
   */
  private async callClaudeAPI(prompt: string): Promise<string> {
    try {
      // Use the Anthropic client from the main app
      ...
      throw new Error('Failed to generate persona using Claude API');
    }
  }
  ...
  private parsePersonaFromText(text: string): Omit<PatientPersona, 'id'> {
    try {
      ...
      console.error('Error parsing persona from text:', error);
      throw new Error('Failed to parse persona from Claude response');
    }
  }
